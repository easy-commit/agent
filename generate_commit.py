...
if __name__ == "__main__":
    repo_path = input("Git repository path to generate a commit message: ").strip()
    ...
    if not diff.strip():
        print("[NO CHANGES] No files staged.")
    else:
        diff_clean = "\n".join(
            line
        ...
        outputs = model.generate(
            input_ids, max_length=128, num_beams=4, early_stopping=True, use_cache=False
        )
